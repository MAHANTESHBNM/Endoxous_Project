...
const CustomerSupport = ({ toggle, setRestrictSide }) => {
  ...
  const { error: actionError, isOpen } = useSelector(
  ...
  const clossedTickets =
    tickets &&
    tickets.filter((ticket) => ticket.ticketClossed?.status === true);
  const newTickets =
    tickets &&
    tickets.filter(
      (ticket) =>
